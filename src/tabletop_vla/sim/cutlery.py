...
# Authored site positions. The live site is read at episode start instead, because the
# drawer workcell relocates `fork_mat` clear of the open drawer (see drawer.py).
MATS = {"fork": np.array([-0.32, 0.10, 0.738]), "spoon": np.array([0.32, 0.10, 0.738])}
# Table top is z 0.735 and the cutlery half-height is 0.003, so a placed piece rests here.
RESTING_Z = 0.738
...
class CutleryRetrieve:
    ...
    def __init__(self, sim, obj="fork", close_gripper=True):
        ...
        site = self.model.site(f"{obj}_mat")
        self.mat = np.array([float(site.pos[0]), float(site.pos[1]), RESTING_Z])
        self.solution = None
        self.drawer_travel_at_handover = 0.0
    ...
    def before_step(self):
        ...
        if self.stage == "drawer":
            ...
            if self.drawer.status == "failed":
                ...
                # Travel is a live reading, and the retrieval arm nudges the drawer partly
                # shut while carrying the cutlery away (measured: 0.0996 down to 0.0616 by
                # the end of the episode). Record what the drawer skill actually achieved,
                # and keep the live value too so the disturbance stays visible.
                self.drawer_travel_at_handover = self.drawer.travel
    ...
    def after_step(self):
        ...
        # The height check is not optional. Without it this gate passed while the fork was
        # resting on the OPEN DRAWER at z 0.79, 60 mm above the table, because the mat's
        # XY lands inside the open drawer footprint (x -0.36..-0.10, y -0.01..0.14).
        # Measured 2026-09-14; the cup scorer has always had the equivalent check at
        # scoring.py:37.
    ...
    def state(self):
        ...
        return {"name": f"{self.obj}-retrieve",
                ...
                "drawer_travel_at_handover_m": self.drawer_travel_at_handover,
